...
export async function handleEditBallotSubmit(fData){
    ...
    try {
      let response;
        ...
        console.log(fData);
        if(fData.ballot_id){
          response = await axios.put(`/api/ballot/update/${[fData.ballot_id, fData.society_id]}`, JSON.stringify(fData), {
            headers: { 
          ...
          response = await axios.post('/api/ballot/create', JSON.stringify(fData), {
            headers: { 
              'Content-Type': 'application/json',
              'Authorization': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsInVzZXJuYW1lIjoidGVzdHVzZXIiLCJpYXQiOjE3NDUwNzQyODksImV4cCI6MTc0NTA3Nzg4OX0.3zvU8-ePlgt77mjw-QBYXRmv3y6cx1L1W1vuYgkaz2M'
            }
...
export async function getAllBallotsBySociety(society_id){
  ...
  ballots.sort((a, b) => new Date(b.endDate) - new Date(a.endDate));
...
export function setStateByPath(setFData, path, value) {
  ...
  export const createFormItem = (stateVar, stateSetter, rel_path, setFData, full_path) => {
    let _path = rel_path + (stateVar.length + 1);
    stateSetter([...stateVar, _path]);
    setStateByPath(setFData, full_path + _path, {});
}

export function syncState(obj, path){
  let fDataAtPath = getValueFromPath(obj, path);
  // console.log("fData at path:");
  // console.log(fDataAtPath);
  let keys = Object.keys(fDataAtPath);
  // console.log("Keys at path:");
  // console.log(keys);
  return keys;
}
...
export const filterBallotsByActive = (ballots) => {
  const now = new Date();
  return ballots.filter(ballot => new Date(ballot.endDate) > now);
}
...
async function loadUserData(userData, role, society_id){
